...
public class UserBuy extends JFrame {
	...
	public UserBuy(String username) {
		...
		try{
			...
			text.append("---- 菜  品 序 号 ----"+" 菜  品  名  称 ----" + " 菜  品 生 产 日 期 ----"+" 会 员 价 格 ----" + " 非 会 员 价 格 ----" + " 菜  品 数 量 ----"+"\n");
			while(res.next()){
		...
		label2.setText("这是所有的菜品信息,购买只需要输入商品序号和购买数量！");
		label2.setFont(new Font("微软雅黑",0,18));
		
		label3.setText("请 输 入 要 购 买 菜  品 的 序 号：");
		label3.setFont(new Font("微软雅黑",0,18));
		
		label4.setText("请 输 入 要 购 买 菜  品 的 数 量：");
	...
	class Myaction extends JFrame implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(goodsid.getgoodsID(id) == id) {
				...
				if(num > n || num == 0) {
					JOptionPane.showMessageDialog(null,"您 购 买 的 菜  品 库 存 不 足 ！", "错误",JOptionPane.ERROR_MESSAGE);
					field.setText("");
					field1.setText("");
				}else {
					...
					if(insert.Goodsinsert(username, id, name, vip, num, cost)) {
						JOptionPane.showMessageDialog(null, name + "  菜  品 购 买 成 功 ！", "购买成功",JOptionPane.DEFAULT_OPTION);
						update.setupgoodsNum(id, n-num);//同时更新商品数量
					}
				}
			}else {
				JOptionPane.showMessageDialog(null,"您 输 入 的 菜  品 序 号 不 存 在 ！", "错误",JOptionPane.ERROR_MESSAGE);
				field.setText("");
				field1.setText("");
			}
		}
